...
const NovoAgendamento = () => {
  ...
  const handleConfirmarAgendamento = async () => {
    ...
    try {
      ...
      setAgendamentoId(result.agendamento.id)
      
      toast.success('Agendamento criado! Prossiga para o pagamento.')
  ...
  const renderSteps = () => {
    const steps = [
      ...
      { number: 6, title: 'Confirmação', completed: agendamentoId !== null },
      { number: 7, title: 'Pagamento', completed: false }
    ]
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Step 7: Pagamento */}
      {step === 7 && showPayment && agendamentoId && (
        <div className="bg-white p-6 rounded-lg shadow-sm border">
          <div className="mb-4">
            <button
              onClick={() => setStep(6)}
          ...
          <div className="space-y-4">
            <div className="bg-gray-50 p-4 rounded-lg mb-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
                <div><span className="text-gray-600">Salão:</span> <span className="font-medium">{selectedSalao.nome}</span></div>
                <div><span className="text-gray-600">Serviço:</span> <span className="font-medium">{selectedServico.nome}</span></div>
                <div><span className="text-gray-600">Profissional:</span> <span className="font-medium">{selectedProfissional.nome}</span></div>
                <div><span className="text-gray-600">Data:</span> <span className="font-medium">{new Date(selectedData).toLocaleDateString('pt-BR')} às {selectedHorario}</span></div>
              </div>
            </div>
