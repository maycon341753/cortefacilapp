 ...
 * Atualizado em: 2025-08-24 13:48:12
 */
...
try {
    ...
    // Buscar horários de funcionamento do salão para o dia da semana
    $dia_semana = date('w', strtotime($data)); // 0=domingo, 1=segunda, etc.
    // Converter para o formato da tabela (1=segunda, 2=terça, etc., 0=domingo=7)
    ...
    $stmt->execute([$salao_id, $dia_semana_db]);
    $funcionamento = $stmt->fetch(PDO::FETCH_ASSOC);
    
    if (!$funcionamento) {
        throw new Exception("Salão fechado neste dia da semana");
    }
    ...
    foreach ($horarios_cadastrados as $horario) {
        ...
        // Verificar se horário está dentro do funcionamento do salão
        $hora_inicio = $horario["hora_inicio"];
        $hora_fim = $horario["hora_fim"];
        
        if ($hora_inicio < $funcionamento["hora_abertura"] || $hora_fim > $funcionamento["hora_fechamento"]) {
            $disponivel = false;
        }
        
        // Verificar se horário está ocupado por agendamento
        if ($disponivel) {
            foreach ($horarios_ocupados as $ocupado) {
                if ($horario["hora_inicio"] === $ocupado["hora"]) {
                    $disponivel = false;
                    break;
                }
            }
        ...
        // Verificar se horário está bloqueado temporariamente
        if ($disponivel) {
            foreach ($bloqueios_temporarios as $bloqueado) {
                if ($horario["hora_inicio"] === $bloqueado["hora_inicio"] && 
                    $horario["hora_fim"] === $bloqueado["hora_fim"]) {
                    $disponivel = false;
                    break;
                }
            }
    ...
    // Extrair apenas os horários para compatibilidade com o frontend
    $horarios_simples = [];
    foreach ($horarios_disponiveis as $horario) {
        $horarios_simples[] = $horario['display'];
    }
    
    // Retornar resposta
    echo json_encode([
        "success" => true,
        "data" => $horarios_simples,
